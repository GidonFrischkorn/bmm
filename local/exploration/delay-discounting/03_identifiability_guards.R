...
cat("==========================================================================\n")
cat("G2: k vs. sensitivity (phi) confound — screen folded into G1\n")
cat("--------------------------------------------------------------------------\n")
cat("Both k (discount rate) and phi (sensitivity) shift the slope of P(LL) vs.\n")
cat("value difference in the linear/short-delay regime. The Hessian-based\n")
cat("correlation check below fires in exactly the same regime as G1 (max\n")
cat("k_ref*D < 0.5): on every demonstrated design G2 fires iff G1 fires.\n")
cat("G1 is the cheaper gate (geometric check, no fitting). G2 survives as\n")
cat("prior guidance: when the linear regime is detected, anchor phi with a\n")
cat("strong prior, or use the Luce rule to drop phi entirely.\n\n")
...
cat("--- G1 vs G2 comparison: G2 screen is redundant ---\n")
cat("  Short-delay (1-7d): G1 ok=FALSE (max kD=0.14),  G2 ok=FALSE (|r|>0.70)\n")
cat("  Adequate (7-365d):  G1 ok=TRUE  (max kD=7.30),  G2 ok=TRUE  (|r|=0.15)\n")
cat("  G2 adds no screening power over G1. G2 screen is folded into G1.\n")
cat("  G2's value: phi-prior baked into configure_model.dd_choice() by default.\n\n")
...
cat("G2 (k vs. phi confound) — screen folded into G1, survives as prior guidance:\n")
cat("  On every demonstrated design G2 fires iff G1 fires — both detect the\n")
cat("  same linear/short-delay regime (max k_ref*D < 0.5). G1 is the single\n")
cat("  gate (geometric check, no Hessian fitting required). G2's independent\n")
cat("  value is the remediation: bake logphi ~ Normal(0.7, 0.3) as the default\n")
cat("  prior in configure_model.dd_choice(), or use Luce rule to drop phi.\n\n")
...
cat("  check_delay_range()       → single gate in check_data.dd_choice() (G1; G2 folded)\n")
cat("  logphi ~ Normal(0.7,0.3) → default prior in configure_model.dd_choice()\n")
