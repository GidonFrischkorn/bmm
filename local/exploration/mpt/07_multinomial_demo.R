...
# Parameters: d_A=0.70, d_B=0.60, g_A=0.40, g_B=0.30, g_New=0.30
# Trees: sourceA, sourceB, new
# Branch expressions:
#   Tree A: P(A)=d_A+(1-d_A)*g_A,  P(B)=(1-d_A)*g_B,  P(New)=(1-d_A)*g_New
#   Tree B: P(A)=(1-d_B)*g_A,      P(B)=d_B+(1-d_B)*g_B, P(New)=(1-d_B)*g_New
#   Tree N: P(A)=g_A,              P(B)=g_B,           P(New)=g_New
# Constraint: g_A + g_B + g_New = 1  (guessing simplex)
# Stick-breaking: g_A=inv_logit(lgA), g_B=(1-g_A)*inv_logit(lgB), g_New=1-g_A-g_B
# =============================================================================
...
# Compute branch probabilities per tree
...
cat(sprintf("True: d_A=%.2f, d_B=%.2f, g_A=%.2f, g_B=%.2f, g_New=%.2f\n",
            dA_true, dB_true, gA_true, gB_true, gNew_true))

# Stick-breaking true values
...
sim_smm <- bind_rows(
  tibble(participant = seq_len(n_participants_b), source = "sourceA") |>
    mutate(
      ...
      New = sapply(counts_raw, `[`, 3), n = n_items_b
    ),
...
out_smm3 <- mpt_to_brms(
  spec_smm3,
  predictor_formulas = list(d_A = ~ 1, d_B = ~ 1, g_A = ~ 1, g_B = ~ 1),
...
# Recovery: back-transform to probability scale
...
cat(sprintf("d_B  : true = %.3f  est = %.3f  95%% CI = [%.3f, %.3f]\n",
            dB_true, dB_hat, dB_ci[1], dB_ci[2]))
cat(sprintf("g_A  : true = %.3f  est = %.3f  95%% CI = [%.3f, %.3f]\n",
            gA_true, gA_hat, gA_ci[1], gA_ci[2]))
cat(sprintf("g_B  : true = %.3f  est = %.3f  (via stick-breaking)\n", gB_true, gB_hat))
cat(sprintf("g_New: true = %.3f  est = %.3f  (derived: 1-gA-gB)\n", gNew_true, gNew_hat))
cat(sprintf("Max R-hat: %.4f\n", max_rhat_smm))
...
cat(sprintf("%-8s  %-6s  %-6.3f  %-12.3f  %.4f\n", "SMM", "d_A",   dA_true,    dA_hat,    max_rhat_smm))
cat(sprintf("%-8s  %-6s  %-6.3f  %-12.3f\n",        "SMM", "d_B",   dB_true,    dB_hat))
cat(sprintf("%-8s  %-6s  %-6.3f  %-12.3f\n",        "SMM", "g_A",   gA_true,    gA_hat))
cat(sprintf("%-8s  %-6s  %-6.3f  %-12.3f\n",        "SMM", "g_B",   gB_true,    gB_hat))
cat(sprintf("%-8s  %-6s  %-6.3f  %-12.3f\n",        "SMM", "g_New", gNew_true,  gNew_hat))
