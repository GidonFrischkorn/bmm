...
results_A <- do.call(rbind, lapply(Filter(Negate(is.null), cov_list_A), function(x) {
  data.frame(model="gains_only", param=x$param, true=x$true, est=x$est,
             q2.5=x$lo, q97.5=x$hi, covered=x$covered,
             max_rhat=rhat_A, n_diverge=div_A)
}))

results_B <- do.call(rbind, lapply(Filter(Negate(is.null), cov_list_B), function(x) {
  data.frame(model="gains_losses_phi1", param=x$param, true=x$true, est=x$est,
             q2.5=x$lo, q97.5=x$hi, covered=x$covered,
             max_rhat=rhat_B, n_diverge=div_B)
}))
...
cat(sprintf("\n  Model B (gains+losses, phi=1, alpha/lambda/gammaw): Rhat=%.3f, div=%d\n",
            rhat_B, div_B))
...
cat("  phi/lambda confound: CONFIRMED — requires phi=1 or informative phi prior\n")
