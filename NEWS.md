...
* The **cswald** model (both versions) gains an optional parameter `sndt` for uniform trial-to-trial variability in the non-decision time (the `st0` of rtdists): the non-decision time is distributed as `Uniform(ndt, ndt + sndt)`, so `ndt` is the minimum and `ndt + sndt/2` the mean non-decision time. `sndt` is fixed to 0 by default, reproducing the previous behaviour exactly, and is estimated by adding a formula for it, e.g. `bmf(..., sndt ~ 1)`. This prevents a handful of fast response times from biasing `ndt`, `drift`, and `bound` (Miller et al., 2017, Fig. 3). `sndt` is weakly identified at typical trial numbers -- its value is in de-biasing the other parameters, not in substantive interpretation -- and for the "crisk" version at substantial error rates it can absorb the crisk-vs-diffusion approximation error; see `?cswald` for both caveats. The distribution functions `dcswald()`, `pcswald()`, `qcswald()`, and `rcswald()` gain a matching `sndt = 0` argument (#387).
...
* Fix the **cswald** survivor with an estimated `sndt` returning noise deep in the right tail. It is a difference of two integrated survivors that both approach `E[D]`, and the guard against their cancellation was absolute rather than relative, so roughly 660 nats of progressively corrupted values passed through before it fired: over one representative grid the log-survivor was non-monotone at 5 of 21 points and its second difference was 1.85 rather than ~1e-4, i.e. the gradient there was zero or garbage. The guard is now relative, matching the one the density already used, and where it fires the same integral is recovered by composite Simpson in log space, whose terms are all positive and so cannot cancel. Maximum error against quadrature drops from ~0.7 to ~0.001 nats (#387).
* `pcswald()` and `qcswald()` now evaluate the lower tail directly instead of as `1 - survivor`, which cancelled away every significant digit for small probabilities -- returning exactly 0 where the true CDF is e.g. 1e-23. The integrated CDF is formed from the same partial expectation as the integrated survivor rather than by subtraction (#387).
* Fix the Stan likelihood of the **cswald** model returning `NaN` for extreme reaction times at high drift rates: `swald_lccdf()` now computes the survivor via `std_normal_lcdf(-z)` instead of `std_normal_lccdf(z)`, which underflows to `-Inf` for `z > ~8.3`, and returns `-Inf` rather than `NaN` when floating-point rounding makes the subtrahend of the survivor difference exceed the minuend. A `NaN` there poisons the gradient of the entire proposal. This is the Stan-side analog of #376 (#387).
...
### Other changes
* The Stan likelihood of the **cswald** model is now evaluated by a vectorized (`loop = FALSE`) custom family instead of a per-observation loop, with numerically identical results — the vectorized form is verified against the scalar Stan likelihood and against the R implementation used by `log_lik()` to ~5e-13 in the test suite. Both model versions now share a single vectorized shifted-Wald density and survivor in `cswald_helper_functions.stan` instead of transcribing the algebra per version. When within-chain threading is requested (via the `threads` argument to `bmm()` or the `brms.threads` option), the family emits sliced data indexing (`dec[start:end]`) so that each `reduce_sum` partial sum pairs its response slice with the matching decisions (#387).
