...
# -----------------------------------------------------------------------------
# cswald: sndt = 0 against an independent implementation, and the guards that
# keep the convolution usable at its boundaries
# -----------------------------------------------------------------------------

test_that("dcswald with sndt = 0 matches an independent inverse-Gaussian density", {
  skip_if_not_installed("statmod")

  # the sndt = 0 likelihood is the one every existing cswald fit uses, so it
  # needs a reference outside bmm; the shifted Wald is an inverse Gaussian with
  # mean bound/drift and shape (bound/s)^2, evaluated at rt - ndt
  drift <- 2
  bound <- 1.2
  ndt <- 0.25
  s <- 0.8
...
test_that(".gwald splits the identity G + H = x and matches quadrature", {
  ...
  # H is the integrated CDF; forming it directly rather than as x - G is what
  # keeps the lower tail from cancelling away entirely
...
test_that("the sndt survivor stays smooth and accurate deep in the tail", {
  # the survivor is a difference of two integrated survivors that both approach
  # E[D]; once it cancels, a naive log() of it is non-monotone noise with a
  # gradient of zero or garbage, which is what produces divergent transitions
  drift <- 3
  bound <- 1
  s <- 0.4
  sndt <- 0.1
...
test_that("the sndt CDF resolves probabilities that 1 - survivor rounds away", {
  ...
  # the route pcswald used before, kept as the reason the argument exists
